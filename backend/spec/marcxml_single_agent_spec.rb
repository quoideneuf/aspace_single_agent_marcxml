...
require_relative '../model/marcxml_accession_converter'

describe 'MARCXML Accession converter' do

  def my_converter
    SingleAgentMarcXMLAccessionConverter
...
END
    ...
    it "only creates a single agent if given a 001 and 700 field" do
      @people.count.should eq(1)
      @accession['linked_agents'].count.should eq(1)

      @people.first['names'].count.should eq(2)
      @people.first['names'].map {|n| n['primary_name']}.sort.should eq(['NAME 1', 'NAME 2'].sort)
